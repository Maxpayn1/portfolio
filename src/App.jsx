...
function Badge({ children }) {
  return <span className="inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium opacity-90">{children}</span>;
}
      ...
      <div className="min-h-screen bg-white text-neutral-900 dark:bg-neutral-950 dark:text-neutral-100 transition-colors">
        ...
        <main id="home" className="mx-auto max-w-6xl px-4 py-10 md:py-14">
          ...
          <Section id="projects" icon={Code2} title="Projets">
            <div className="mb-4">
              <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Rechercher titre, techno..." className="w-full rounded-xl border bg-transparent px-3 py-2 text-sm" />
              <div className="mt-3 flex flex-wrap gap-2">
                {types.map((t) => (
                  <button key={t} onClick={() => setType(t)} className={`rounded-full border px-3 py-1 text-xs ${type === t ? "bg-black text-white dark:bg-white dark:text-black" : "hover:shadow"}`}>
                    {t}
                  </button>
                ))}
              </div>
